...
def render_details_tab(session):
    ...
    col_title, col_refresh = render_refresh_button("tab_details")
    with col_title:
        st.markdown("### 📊 Application Data Explorer")
    with col_refresh:
    ...
    with st.expander("🗄️ Database Retention Details", expanded=False):
        try:
            query = """
            SELECT 
    ...
    with st.expander("⚙️ Configuration Rules", expanded=False):
        try:
            query = """
            SELECT 
    ...
    with col1:
        try:
            count_query = "SELECT COUNT(*) as cnt FROM data_schema.warehouse_details"
            count = session.sql(count_query).collect()[0]['CNT']
            st.metric("Warehouse Records", count)
        except:
            st.metric("Warehouse Records", "Error")
    
    with col2:
        try:
            count_query = "SELECT COUNT(*) as cnt FROM data_schema.database_retention_details"
            count = session.sql(count_query).collect()[0]['CNT']
            st.metric("Database Retention Records", count)
        except:
            st.metric("Database Retention Records", "Error")
    
    with col3:
        try:
            count_query = "SELECT COUNT(*) as cnt FROM data_schema.config_rules WHERE is_active = TRUE"
            count = session.sql(count_query).collect()[0]['CNT']
            st.metric("Active Rules", count)
        except:
            st.metric("Active Rules", "Error")
    
    with col4:
        try:
            count_query = "SELECT COUNT(*) as cnt FROM data_schema.applied_rules WHERE is_active = TRUE"
            count = session.sql(count_query).collect()[0]['CNT']
            st.metric("Applied Rules", count)
        except:
            st.metric("Applied Rules", "Error")
